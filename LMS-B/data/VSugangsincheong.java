...
public class VSugangsincheong {
	...
	public void show(OMember oMember) {
		System.out.println(oMember.getName() + "´Ô! ¾È³çÇÏ¼¼¿ä");	
		String collegeFile = this.vCampus.show(ROOT_FILENAME, "Ä·ÆÛ½º¸¦");
		if (collegeFile != null) {
			String departmentFile = this.vCollege.show(collegeFile, "´ëÇÐÀ»");
			if (departmentFile != null) {
				String lectureFileName = this.vDepartment.show(departmentFile, "ÇÐ°ú¸¦");
				if (departmentFile != null) {
					OLecture oLecture = this.vLecture.show(lectureFileName, "°­ÁÂ¸¦");
				}
			}
		}
	}
